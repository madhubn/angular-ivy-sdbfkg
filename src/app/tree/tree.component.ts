...
import {
  ExpandableArrowPlacement,
  ExpandableType,
  TreeTableData,
  TreeTableDataConfig,
  TreeTableHeaderObject,
  TreeTableRow,
} from 'angular-tree-table';
...
export class TreeComponent implements OnInit {
  ...
  loadData() {
    ...
    this.http
      .get('http://example.com/message/pdsdata/versand_monitor')
      .subscribe((resp) => {
        ...
        for (let level1Key in resp['messageResult']) {
          ...
          let row = new TreeTableRow(
            level1Key,
            {
              name: level1Key,
              data: this.addMissedMonths(rowData['null']['null']['null']),
            },
            true,
            undefined
          );
          ...
          for (let level2Key in rowData) {
            ...
            let subRow = new TreeTableRow(
              level2Key,
              {
                name: level2Key,
                data: this.addMissedMonths(subRowData['null']['null']),
              },
              true,
              undefined
            );
            ...
            for (let level3Key in subRowData) {
              ...
              let subSubRow = new TreeTableRow(
                level3Key,
                {
                  name: level3Key,
                  data: this.addMissedMonths(subSubRowData['null']),
                },
                true,
                undefined
              );
              ...
              for (let accountName in subSubRowData) {
                ...
                let subSubSubRow = new TreeTableRow(
                  accountName,
                  {
                    name: accountName,
                    data: this.addMissedMonths(subSubSubRowData),
                  },
                  false,
                  undefined
                );
  ...
  populateHeaders() {
    ...
    this.tableHeaders.push(
      new TreeTableHeaderObject('Name', 'name', null, true)
    );
    for (let month of this.months) {
      this.tableHeaders.push(
        new TreeTableHeaderObject(month, 'data.' + month, null, true)
      );
    }
